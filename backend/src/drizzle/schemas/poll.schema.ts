...
export const response = pgTable(
  ...
  (table) => [
    ...
    unique("response_poll_respondent_unique").on(table.pollId, table.respondentId).nullsNotDistinct(),
    unique("response_poll_session_unique").on(table.pollId, table.sessionToken).nullsNotDistinct(),
  ]
);
...
export const responseRelations = relations(response, ({ one, many }) => ({
  ...
  respondent: one(user, { fields: [response.respondentId], references: [user.id] }),
  answers: many(responseAnswer),
}));

export const responseAnswerRelations = relations(responseAnswer, ({ one }) => ({
  response: one(response, { fields: [responseAnswer.responseId], references: [response.id] }),
  question: one(question, { fields: [responseAnswer.questionId], references: [question.id] }),
  option: one(option, { fields: [responseAnswer.optionId], references: [option.id] }),
}));
